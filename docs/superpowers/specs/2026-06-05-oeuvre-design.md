...
- **active cycle** (`advance()`), in order:
  ...
  5. Otherwise run a **leader-pick** call (`runAdapter`, `OEUVRE_LEADER_PICK_TIMEOUT_MS`). Parse `<<NEXT>>`:
     ...
     - invalid pick or leader-call failure → `leader_failures++`; if `>= max_consecutive_failures` ⇒ `paused`; else `advance()` again (one re-ask). Every failed attempt is **diagnosed**: a specific reason — no usable adapter, adapter timed out, adapter exited non-zero (with a stderr snippet), no `<<NEXT>>` directive emitted (with an output snippet), leader picked itself, or picked a non-participant — is appended as a `note` event and carried into the pause as `pause_reason="leader_unavailable: <reason>"`, so the director sees *why*, not just *that*, the leader stalled.
